...
const files = [
  ...
  path.join(root, "prisma", "unikassa-template.db"),
];
...
for (const f of files) {
  try {
    if (fs.existsSync(f)) {
      fs.unlinkSync(f);
      console.log("[uniKassa] Removed", path.relative(root, f));
    }
  } catch (e) {
    console.error("[uniKassa] Could not remove", f, e.message);
    process.exit(1);
  }
}
...
console.log("\n[uniKassa] Database reset complete.");
